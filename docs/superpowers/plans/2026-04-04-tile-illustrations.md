# Catan Tile Illustrations Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace solid-color hex tiles with flat-style SVG-illustrated tiles that visually represent each Catan resource type.

**Architecture:** A new file `tile-illustrations.tsx` contains inline SVG React components for each resource type. The existing `catan-generator.tsx` imports the dispatcher component and renders it inside each hex div, replacing the solid color background with a gradient + illustration layer.

**Tech Stack:** React, TypeScript, inline SVG, Tailwind CSS, CSS clip-path
...
|------|--------|----------------|
| `src/themes/default/blocks/tile-illustrations.tsx` | **Create** | All SVG illustration components + TileIllustration dispatcher |
| `src/themes/default/blocks/catan-generator.tsx` | **Modify** | Import illustrations, add gradients, update hex rendering |

---

### Task 1: Create tile-illustrations.tsx with all SVG components

**Files:**
- Create: `src/themes/default/blocks/tile-illustrations.tsx`

- [ ] **Step 1: Create the file with all 6 resource SVG components and the dispatcher**

Create `src/themes/default/blocks/tile-illustrations.tsx` with this content:
...
- [ ] **Step 2: Commit the tile illustrations file**

```bash
git add src/themes/default/blocks/tile-illustrations.tsx
git commit -m "feat: add SVG tile illustrations for all 6 Catan resource types"
```

---

### Task 2: Integrate illustrations into the board generator

**Files:**
- Modify: `src/themes/default/blocks/catan-generator.tsx`

- [ ] **Step 1: Add RESOURCE_GRADIENTS constant and import TileIllustration**

At the top of `catan-generator.tsx`, add the import and the new gradient constant.

After line 3 (`import { useState, useEffect } from 'react';`), add:
```tsx
import { TileIllustration } from './tile-illustrations';
```

After the existing `RESOURCE_COLORS` block (line 46), add:
```tsx
...
Update `RESOURCE_COLORS` to use the `from` colors for the legend (replace lines 39-46).

Note: This intentionally changes all legend swatch colors to match the new gradient backgrounds. The old pastel colors will be replaced with richer tones that match the illustrated tiles.
```tsx
...
- [ ] **Step 2: Update hex tile rendering to use gradient + illustration**

Replace the hexagon div (lines 374-403) with this updated version that uses gradient backgrounds and renders the TileIllustration SVG:

Find and replace the hexagon section:
```tsx
                  {/* Hexagon */}
                  <div
...
Key changes:
- `backgroundColor` → `background: linear-gradient(...)` using `RESOURCE_GRADIENTS`
- Added `overflow-hidden` class
- Added `<TileIllustration>` layer with `opacity-[0.85]` (arbitrary value for Tailwind v4) and `pointer-events-none`
- Desert emoji removed (DesertIllustration handles it via TileIllustration dispatcher)
- Number chit gets `z-10` and `shadow-sm` for readability over illustrations

- [ ] **Step 3: Verify the build compiles**

```bash
cd /home/moses/claude_workspace/catan_board_generator_online/catan-board-generator && npm run build 2>&1 | head -30
```
...
```bash
git add src/themes/default/blocks/catan-generator.tsx
git commit -m "feat: integrate SVG tile illustrations into board generator"
```

---

### Task 3: Visual verification and final adjustments

- [ ] **Step 1: Start the dev server and visually verify**

```bash
cd /home/moses/claude_workspace/catan_board_generator_online/catan-board-generator && npm run dev
```

Check:
- [ ] Each resource type shows a distinct illustration (trees for wood, sheep for wool, etc.)
- [ ] Number chits are clearly readable on top of illustrations
- [ ] Both normal and expanded modes render correctly
- [ ] Desert tile shows cactus/dune illustration (no emoji)
- [ ] Legend at bottom still shows color swatches correctly
- [ ] Resource labels below tiles are still visible

- [ ] **Step 2: If visual issues found, adjust SVG colors/opacity/sizing and commit fixes**

- [ ] **Step 3: Push to remote**

```bash
cd /home/moses/claude_workspace/catan_board_generator_online/catan-board-generator && git push
```
